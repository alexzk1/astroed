...
namespace chkenum
{
    ...
    public:
        ...
        static
        typename std::enable_if<std::is_enum<T>::value, CPairRef>::type
        getEnumFirstLast()
        {
...
template<class T>
inline
typename std::enable_if<std::is_enum<T>::value, typename chkenum::EnumCheck<T>::CPairRef>::type
getEnumFirstLast()
{
...
forEachEnum(const typename std::function<void(T)>& callback)
{
...
inline
typename std::enable_if<std::is_enum<T>::value, void>::type
forEachEnumBreakable(const typename std::function<bool(T)>& callback)
{
    auto range = getEnumFirstLast<T>();
    for (int64_t index = static_cast<decltype (index)>(range.first); index <= static_cast<decltype (index)>(range.second); index ++)
