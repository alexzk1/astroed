...
//            }
    ...
    public:
        ...
        THRNM::thread & get_thread(int i) const { return *this->threads[i]; }
        ...
        auto pushRun(executor_t && f) ->std::future<void>
        {
            auto r = []()->bool{return true;};
            return push(std::forward<executor_t>(f), r);
        }
    ...
    private:
        ...
        void set_thread(int i)
        {
            ...
            auto f = [this, i, flag/* a copy of the shared ptr to the flag */]()
            {
                ...
                while (true)
                {
                    ...
                    this->cv.wait(lock, [this, &fr, &isPop, &_flag]() {
                        isPop = this->q.pop(fr); return isPop || this->isDone || _flag;
                    });
                    --this->nWaiting;
                    if (!isPop)
            ...
            attr.set_stack_size(1024*1024*2);
            this->threads[i].reset(new boost::thread(attr, f)); // compiler may not support std::make_unique()
#else
            this->threads[i].reset(new std::thread(f));
#endif
        }

        void init() { this->nWaiting = 0; this->isStop = false; this->isDone = false; isStopRequested = false;}

        std::vector<std::unique_ptr<THRNM::thread>> threads;
