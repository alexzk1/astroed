    ...
    while (begin < end)
    {
        ...
        if (t[mid].last < ch)
            ...
            if (t[mid].first > ch)
                ...
                if ((ch - t[mid].first) % t[mid].step == 0)
...
static int Lutf8_codes(lua_State *L)
{
    ...
    static std::function<int(lua_State*L)> tmp(Lutf8_next);
    lua_pushcfunction(L, &tmp);
    lua_pushvalue(L, 1);
    lua_pushinteger(L, 0);
    return 3;
}
...
static int Lutf8_gmatch(lua_State *L)
{
    ...
    lua_settop(L, 2);
    lua_pushinteger(L, 0);
    static std::function<int(lua_State*L)> tmp(gmatch_aux);
    lua_pushcclosure(L, &tmp, 3);
    return 1;
}
...
LUALIB_API int luaopen_utf8c(lua_State *L)
{
