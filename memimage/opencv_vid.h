...
private:
    ...
    std::atomic<bool> rOk;
public:
    VideoFileRead(std::string  fileName):
    ...
    bool read(long frame, cv::Mat& res)
    {
        ...
        if (!thr)
        {
            thr = startNewRunner([this](const auto should_stop)
            {
                ...
                framesCount = static_cast<long>(vid.get(cv::CAP_PROP_FRAME_COUNT));
                auto val    = static_cast<uint32_t>(vid.get(cv::CAP_PROP_FOURCC));
                ...
                while (!(*should_stop))
                {
                    ...
                    {
                        if (static_cast<long>(vid.get(cv::CAP_PROP_POS_FRAMES)) != frame)
